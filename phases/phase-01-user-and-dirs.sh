#!/usr/bin/env bash
# mangos-installer — phase 01: create mangos user and directory tree
# SPDX-License-Identifier: GPL-2.0-only
# See CLAUDE.md for design, README.md for usage
# Stub for milestone 1; full implementation lands in milestone 2.

run_phase_01() {
  MANGOS_CURRENT_PHASE="phase-01-user-and-dirs"
  if state_has_completed "$MANGOS_CURRENT_PHASE"; then
    ui_phase_header 1 14 "user and dirs (already done — skipping)"
    return 0
  fi
  ui_phase_header 1 14 "user and dirs"
  ui_status_info "stub: create mangos user, scaffold ~/mangos/{opt,.installer,<realm>}/ — milestone 2"
  state_mark_complete "$MANGOS_CURRENT_PHASE"
}
